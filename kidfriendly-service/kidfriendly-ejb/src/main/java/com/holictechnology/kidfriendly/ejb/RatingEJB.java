...
public class RatingEJB extends AbstractEJB implements RatingLocal {
     ...
     * 
     * @see
     * com.holictechnology.kidfriendly.ejb.interfaces.RatingLocal#listByCompany(
     * java.lang.Long,
     * com.holictechnology.kidfriendly.domain.dto.paginator.PaginatorDto)
     */
    @Override
    @Transactional(value = TxType.NOT_SUPPORTED)
     ...
     * (non-Javadoc)
     * 
     * @see com.holictechnology.kidfriendly.ejb.interfaces.RatingLocal#
     * calculateRating(com.holictechnology.kidfriendly.domain.entity.Company)
     */
    @Asynchronous
    ...
    private StringBuffer createHql(Long idCompany, Boolean stActive, Boolean stDelete, boolean isOrderBy) {
        ...
        hql.append("SELECT new com.holictechnology.kidfriendly.domain.dto.RatingDto(rating.idRating, rating.dtRating, rating.desRating, rating.desAnswer, "
                + "statusKidFriendly.idStatusKidFriendly, company.idCompany, company.desName, user.idUser, user.dsName, user.mgUser) ");
        hql.append("FROM Rating AS rating ");
